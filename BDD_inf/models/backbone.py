...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.pruned_channel_planes = int(planes - math.floor(planes *
                                                             pruning_rate))
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, pruning_rate):
        ...
        self.layer2 = self._make_layer(
            block, 128, layers[1], pruning_rate, stride=2)
        self.layer3 = self._make_layer(
            block, 256, layers[2], pruning_rate, stride=2)
        self.layer4 = self._make_layer(
            block, 512, layers[3], pruning_rate, stride=2)

        self.aggregation = AggregationBlock(
            n_channels1=128,
            n_channels2=256,
            n_channels3=512,
            det_features=128)
    ...
    def forward(self, x):
        ...
        c3 = self.aggregation.forward(c3=C3, c4=C4, c5=C5)
...
class AggregationBlock(nn.Module):
    def __init__(self, n_channels1, n_channels2, n_channels3,
        ...
        self.squeeze1 = nn.Conv2d(
            in_channels=n_channels1, out_channels=det_features, kernel_size=1)
        self.squeeze2 = nn.Conv2d(
            in_channels=n_channels2, out_channels=det_features, kernel_size=1)
        self.squeeze3 = nn.Conv2d(
            in_channels=n_channels3, out_channels=det_features, kernel_size=1)

        self.node1 = nn.Sequential(
            nn.Conv2d(
                in_channels=det_features,
                out_channels=det_features,
                kernel_size=3,
                padding=1),
            nn.BatchNorm2d(num_features=det_features),
    ...
    def forward(self, c3, c4, c5):
        ...
        c3 = self.node1.forward(c3 + F.upsample(
            c4, scale_factor=2, mode='bilinear'))
        c4 = self.node2.forward(c4 + F.upsample(
            c5, scale_factor=2, mode='bilinear'))
        c3 = self.node3.forward(c3 + F.upsample(
            c4, scale_factor=2, mode='bilinear'))

        c3 = torch.cat([
            c3, F.upsample(
                c4, scale_factor=2, mode='bilinear'), F.upsample(
                    c5, scale_factor=4, mode='bilinear')
        ],
                       dim=1)

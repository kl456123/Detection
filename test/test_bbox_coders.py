...
def test_mobileye_coder():
    ...
    for sample in dataset:
        ...
        label_boxes_3d = torch.stack(
            1 * [label_boxes_3d[:num_instances]], dim=0)
        label_boxes_2d = torch.stack(
            1 * [label_boxes_2d[:num_instances]], dim=0)
        proposals = torch.stack(1 * [proposals[:num_instances]], dim=0)
        image_info = torch.stack(1 * [image_info], dim=0)
        p2 = torch.stack(1 * [p2], dim=0)

        encoded_corners_2d = bbox_coder.encode_batch(
            label_boxes_3d, label_boxes_2d, p2, image_info, label_boxes_2d)
        #  torch.cat([encoded_corners_2d, ])
        # num_boxes = encoded_corners_2d.shape[1]
        # batch_size = encoded_corners_2d.shape[0]
        # center_depth = encoded_corners_2d[:, :, -1:]
        ...
        image_path = sample[constants.KEY_IMAGE_PATH]
        visualizer.render_image_corners_2d(
            image_path, corners_2d=decoded_corners_2d[0], p2=p2[0])


def test_grnet_coder():

    coder_config = {'type': constants.KEY_CORNERS_3D_GRNET}
    bbox_coder = bbox_coders.build(coder_config)
    ...
    for sample in dataset:
        ...
        import ipdb
        ipdb.set_trace()
        encoded_corners_2d = bbox_coder.encode_batch(label_boxes_3d, p2)
        center_cylinder_2d = encoded_corners_2d[:, :, 25:27]

        boxes_2d = torch.cat(
            [
                center_cylinder_2d, center_cylinder_2d + 100,
                torch.ones_like(center_cylinder_2d[..., -1:])
            ],
            dim=-1)
        # boxes_2d = torch.cat([proposals, torch.ones_like(proposals[:, :, -1:])], dim=-1)

        corners_3d = bbox_coder.decode_batch(encoded_corners_2d, proposals, p2)

        corners_3d = corners_3d.cpu().detach().numpy()

        #  corners_3d_orig = geometry_utils.torch_boxes_3d_to_corners_3d(label_boxes_3d[0]).cpu().detach().numpy()[None]
        boxes_2d = boxes_2d.cpu().detach().numpy()

        image_path = sample[constants.KEY_IMAGE_PATH]
        image = np.array(sample[constants.KEY_IMAGE])
        visualizer.render_image_corners_2d(
            image_path,
            # image=image,
            corners_3d=corners_3d[0],
            p2=p2[0],
            boxes_2d=boxes_2d[0])

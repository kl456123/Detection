#include <THC/THC.h>
#include <math.h>
#include "cuda/psroi_align_kernel.h"



extern THCState* state;

int psroi_align_forward_cuda(int pooled_height, int pooled_width, float spatial_scale, int group_size, int output_dim,THCudaTensor *features, THCudaTensor* rois, THCudaTensor* output, THCudaIntTensor* mappingchannel){
	...
	PSROIAlignForwardLauncher(data_in, spatial_scale, num_rois, data_height, data_width, num_channels, pooled_height, pooled_width,rois_in, group_size,
	output_dim, output_out, mappingchannel_out,stream);
	return 1;
}


int psroi_align_backward_cuda(int pooled_height, int pooled_width, float spatial_scale, int output_dim,
THCudaTensor* top_grad, THCudaTensor* rois, THCudaTensor* bottom_grad, THCudaIntTensor* mappingchannel)
{
	...
	float *bottom_grad_flat = THCudaTensor_data(state, bottom_grad);
    	...
    	PSROIAlignBackwardLauncher(top_grad_flat, mappingchannel_flat, batch_size, num_rois, spatial_scale, num_channels, data_height, data_width, pooled_width,	      pooled_height, output_dim, bottom_grad_flat, rois_flat, stream);
        return 1;
}

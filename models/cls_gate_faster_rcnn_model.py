...
from core.cls_gate_target_assigner import TargetAssigner
...
class ClsGateFasterRCNN(Model):
    def forward(self, feed_dict):
        ...
        # shape(N,C,1,1)
        pooled_feat_bbox = self.feature_extractor.second_stage_feature(
            pooled_feat)

        # import ipdb
        # ipdb.set_trace()
        if self.training:
            cls_gate = prediction_dict['cls_gate'].unsqueeze(1)
            cls_gate = cls_gate.view(cls_gate.shape[0], -1, self.pooling_size,
                                     self.pooling_size)
            pooled_feat_cls = cls_gate * pooled_feat + (
                (1 - cls_gate) * pooled_feat).detach()
        ...
        pooled_feat_bbox = pooled_feat_bbox.mean(3).mean(2)
    ...
    def init_modules(self):
        ...
        #  self.rcnn_cls_pred = nn.Conv2d(2048, self.n_classes, 1, 1, 0)
        if self.reduce:
            in_channels = 2048
        else:
    ...
    def pre_subsample(self, prediction_dict, feed_dict):
        ...
        #  import ipdb
        #  ipdb.set_trace()
        rcnn_cls_targets, rcnn_reg_targets, rcnn_cls_weights, rcnn_reg_weights, cls_gate = self.target_assigner.assign(
        ...
        prediction_dict['rcnn_reg_targets'] = rcnn_reg_targets[
            batch_sampled_mask]
        prediction_dict['cls_gate'] = cls_gate[batch_sampled_mask]

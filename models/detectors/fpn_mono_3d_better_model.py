# -*- coding: utf-8 -*-
"""
dont expolit 2d projection, just predict others to get location,
predict direction keypoints to get the orientation
"""
...
@DETECTORS.register('fpn_mono_3d_better')
class FPNMono3D(FPNFasterRCNN):
    def forward(self, feed_dict):
        ...
        for i in range(self.num_stages):
            ...
            if self.training:
                ...
                gt_dict[constants.KEY_MONO_3D_NON_2D_PROJ] = None
            ...
            if self.training:
                ...
                loss_units[constants.KEY_MONO_3D_NON_2D_PROJ][
                    'pred'] = rcnn_orient_preds
            ...
            coder = bbox_coders.build({
                'type': constants.KEY_MONO_3D_NON_2D_PROJ
            })
            # use rpn proposals to decode
            rcnn_mono_3d = coder.decode_batch(
                rcnn_orient_preds, proposals,
                feed_dict[constants.KEY_STEREO_CALIB_P2]).detach()

            rcnn_mono_3d = torch.stack([rcnn_mono_3d, rcnn_dim_preds], dim=1)
        ...
        if self.training:
            ...
            prediction_dict[constants.
                            KEY_MONO_3D_NON_2D_PROJ] = rcnn_orient_preds
            ...
            prediction_dict[constants.KEY_MONO_3D_NON_2D_PROJ] = rcnn_mono_3d
    ...
    def init_weights(self):
        ...
        self.freeze_modules()
        for param in self.rcnn_orient_preds.parameters():
            param.requires_grad = True

        for param in self.rcnn_dim_preds.parameters():
            param.requires_grad = True

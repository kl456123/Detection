...
class MultiBinLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        Args:
            preds: shape(N,M,num*4)
            targets: shape(N,M,1)
        """
        batch_size = preds.shape[0]
        preds = preds.contiguous().view(batch_size, -1, self.num_bins, 4)
        ...
        angle_cls_loss = angle_cls_loss.view(batch_size, -1, self.num_bins)
        # change from sum to mean
        angle_cls_loss = angle_cls_loss.sum(dim=-1, keepdim=True)

        # residual loss
        # reg_targets = self.generate_reg_targets(targets)
        theta = torch.atan2(preds[:, :, :, 3], preds[:, :, :, 2])
        ...
        angle_reg_loss = 1 / num_covered.unsqueeze(-1) * angle_reg_loss.sum(
            dim=-1, keepdim=True)

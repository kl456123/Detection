...
def _get_image_blob(im):
  ...
  """
    ...
    # modifyied here for compatite with training code
    im_orig = im.astype(np.float32, copy=True)
    im_orig /= 255
    im_orig = im_orig - DATA_AUG_CFG['normal_mean']
    im_orig /= DATA_AUG_CFG['normal_van']
...
def generate_imglist(datafile):
    with open(datafile, 'r') as f:
        lines = f.readlines()
        lines = [line.strip() + '.png' for line in lines]
    return lines


def generate_result(all_dets):
...
if __name__ == '__main__':
    ...
    pascal_classes = np.asarray(['bg', 'car'])
    ...
    if args.cuda > 0:
        fasterRCNN = nn.DataParallel(fasterRCNN, device_ids=[0])
        fasterRCNN.cuda()
    ...
    if webcam_num >= 0:
        ...
        # gengerate img list for test
        # datafile = '/data/object/training/val.txt'
        datafile = './val_car.txt'
        imglist = generate_imglist(datafile)
    ...
    for i in range(num_images):
        ...
        for j in xrange(1, len(pascal_classes)):
            ...
            if inds.numel() > 0:
                ...
                all_dets.append(cls_dets.cpu().numpy())
            else:
                # print("no detection")
        ...
        if vis and webcam_num == -1:
            ...
            result_path = os.path.join(args.image_dir,
                                       imglist[num_images][:-4] + "_det.jpg")
            cv2.imwrite(result_path, im2show)
        det_file = 'results/detections.pkl'
    with open(det_file, 'wb') as f:
        pickle.dump(all_dets, f, pickle.HIGHEST_PROTOCOL)
        # else:

...
import data.data_loader_old as data
...
def generate_datafile(datafile, img_names):
    with open(datafile, 'w') as f:
        img_names = [
            os.path.splitext(img_name)[0][-6:] for img_name in img_names
        ]
        f.write('\n'.join(img_names))
...
BATCH_SIZE = 1
model_root_path = '/data/object/liangxiong/models/'
...
if __name__ == '__main__':
    ...
    for i in range(num_images):
        ...
        if max_per_image > 0:
            image_scores = np.hstack(
                [all_boxes[j][i][:, -1] for j in xrange(1, len(classes))])
    ...
    generate_datafile('val_car.txt', img_filenames)
